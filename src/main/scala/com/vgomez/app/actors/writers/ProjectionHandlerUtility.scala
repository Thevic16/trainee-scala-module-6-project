...
object ProjectionHandlerUtility {
  def getRestaurantModelByRegisterRestaurantState(registerRestaurantState: RegisterRestaurantState)
    : RestaurantModel = {
    RestaurantModel(Some(registerRestaurantState.index), registerRestaurantState.id,
      registerRestaurantState.username,
  ...
  def getReviewModelByRegisterReviewState(registerReviewState: RegisterReviewState): ReviewModel = {
    ReviewModel(Some(registerReviewState.index), registerReviewState.id, registerReviewState.username,
      registerReviewState.restaurantId, registerReviewState.stars, registerReviewState.text,
      registerReviewState.date)
  }
